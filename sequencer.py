...
class SequenceState(Enum):
    PREPARING = "PREPARING"  # Wait for all items to be present
    IDLE = "IDLE"           # Ready to start sequence
    TRACKING = "TRACKING"   # Sequence in progress
    VALIDATING = "VALIDATING" # Wrong item detected
...
class Sequencer:
    ...
    def _is_colliding_with_border(self, object_box: Tuple[int, int, int, int], border_box: Tuple[int, int, int, int]) -> bool:
        """
        Check if object is crossing/touching the border.
        Returns True if object is touching the edge (crossing).
        Returns False if object is completely inside or completely outside (no overlap).
        """
        x1, y1, x2, y2 = object_box
        bx1, by1, bx2, by2 = border_box
        tol = self.tolerance
        
        # Check if object is completely inside (with tolerance)
        completely_inside = (
            x1 > bx1 + tol and 
            x2 < bx2 - tol and 
            y1 > by1 + tol and 
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, str]:
        ...
        # 1. PREPARING State: Wait for ALL items to be present
        if self.current_state == SequenceState.PREPARING:
            # Check if all unique items in sequence are present in frame
        ...
        # 2. COMPLETED State: Wait for reset (e.g., items put back)
        elif self.current_state == SequenceState.COMPLETED:
            # Auto-reset logic: If we see items coming back, or just wait?
            # Let's say we go back to PREPARING immediately to wait for next batch
            # Or wait for frame to clear? User said "if all object is all in frames together... Clear"
            # Actually, user said: "if all object is all in frames together of all thing in list... stay Clear"
            # "and if all things got detect inside then we proceed to idle"
            
            # So: COMPLETED -> Wait until all items are back in frame -> IDLE?
            # Let's reuse PREPARING logic for that.
            self._transition_to(SequenceState.PREPARING, "Sequence finished, resetting")
            
        # 3. Active States (IDLE, TRACKING, VALIDATING)
        else:
            # Detect NEW border crossings (Inside -> Outside)
            # A class is "crossing" if it is on border NOW but wasn't PREVIOUSLY
            # This handles the "trail" issue naturally
            ...
            for class_name in new_crossings:
                ...
                if self.expected_index < len(self.tracking_list):
                    ...
                    if class_name == expected_class:
                        ...
                        # Wrong item leaving
                        # Only warn if it's not a duplicate of what we just did?
    ...
    def _draw_status(self, frame, message, height):
        color_map = {
            SequenceState.PREPARING: (0, 255, 255), # Yellow
            SequenceState.IDLE: (200, 200, 200),    # Gray
            SequenceState.TRACKING: (0, 255, 0),    # Green
            SequenceState.VALIDATING: (0, 0, 255),  # Red
            SequenceState.COMPLETED: (255, 165, 0)  # Orange
        }
    ...
    def get_display_message(self) -> str:
        if self.current_state == SequenceState.PREPARING:
            ...
            # Fix counter: Show "Progress: 1/3" when 1 is done
            # expected_index is 0-based. If 1 item done, index is 1.
            # So "Progress: {index}/{total}" is correct for "items completed"
    ...
    def get_state_info(self) -> dict:
        return {
            ...
            'validation_time_remaining': 0 # Simplified
        }